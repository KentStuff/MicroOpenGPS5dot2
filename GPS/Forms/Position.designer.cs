...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        public vec2[] fixFilter = new vec2[10];
        ...
        public void UpdateFixPosition()
        {
            ...
            if (Math.Abs(pn.speed) < .5 && isSlowFilter)
            {
                fixFilter[0] = fixFilter[1];
                fixFilter[1] = fixFilter[2];
                fixFilter[2] = fixFilter[3];
                fixFilter[3] = fixFilter[4];
                fixFilter[4] = fixFilter[5];
                fixFilter[5] = fixFilter[6];
                fixFilter[6] = fixFilter[7];
                fixFilter[7] = fixFilter[8];
                fixFilter[8] = fixFilter[9];


                fixFilter[9] = pn.fix;
                if (fixFilter[0].easting != 0 || fixFilter[0].northing != 0)
                {
                    double tempEasting = 0;
                    double tempNorthing = 0;
                    for (int i = 0; i < 10; i++)
                    {
                        tempEasting += fixFilter[i].easting;
                        tempNorthing += fixFilter[i].northing;

                    }
                    fixFilter[0].easting = 0;
                    fixFilter[0].northing = 0;
                    pn.fix.easting = tempEasting / 10;
                    pn.fix.northing = tempNorthing / 10;
                }
                else
                { TheRest(); }
                ...
                case "Fix":
                    {
                        ...
                        if (Math.Abs(avgSpeed) < startSpeed && !isFirstHeadingSet)
                            goto byPass;
                        ...
                        else
                        {
                            ...
                            if (distanceCurrentStepFix > minFixStepDist/2)
                            {
                                //most recent heading
